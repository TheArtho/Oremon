...
export function UseOreball(event) {
    ...
    if (BattleManager.isInBattle(player.id))
        return;
...
export function startBattleWithEntity(player, entity, projectile) {
    ...
    if (!battle) {
        ...
        // Set up the battle
        const playerTrainer = {
            type: "trainer", active: MathUtils.clamp(player.selectedSlotIndex, 0, PlayerSave.data.get(player.id).getTeam().filter(o => o !== undefined).length - 1), player: player, team: PlayerSave.data.get(player.id).getTeam().filter(Boolean)
        };
        const opponent = {
