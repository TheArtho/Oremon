...
export class PlayerMonEntityManager {
    static playerSendOut(player: Player, slot: number, location: Vector3): Entity | undefined {
    ...
    static playerRetrieve(player: Player, slot: number) {
        const playerData = PlayerSave.data.get(player.id);
        if (!playerData) return;
        ...
        system.run(() => {
            ...
            const entities = world.getDimension(player.dimension.id)
                ...
                .filter(entity =>
                    entity.getTags().includes(`player:${player.id}`) &&
